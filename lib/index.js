...
module.exports = async config => {
  ...
  let { include, exclude, outDir, markdownDir, babelParserPlugins } = config
  ...
  const mdTemplate = await majo.fs.readFile(
    path.resolve(__dirname, './templates/md.md'),
    'utf-8'
  )
  const nameRE = /\[name\]/g
  ...
  return files.map(async p => {
    ...
    try {
      ...
      logger.success(
        `Successfully ${alreadyExists ? 'updated' : 'created'}: ${target}`
      )
      return compName
    } catch (e) {
